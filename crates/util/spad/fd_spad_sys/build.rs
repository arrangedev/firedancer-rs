...
fn main() {
    ...
    let spad_path = util_path.join("spad");
    ...
    println!(
        ...
        spad_path.join("fd_spad.c").display()
    );
    println!(
        "cargo:rerun-if-changed={}",
        spad_path.join("fd_spad.h").display()
    );
    println!(
        "cargo:rerun-if-changed={}",
    ...
    let wrapper_path = out_path.join("spad_wrapper.h");
...
#include "{}/fd_spad.h"
"#,
            spad_path.canonicalize().unwrap().display()
        ),
    )
    ...
    let mut bindgen = bindgen::Builder::default()
        ...
        .allowlist_function("fd_spad_.*")
        .allowlist_type("fd_spad_.*")
        .allowlist_type("fd_valloc_.*")
        .allowlist_var("FD_SPAD_.*")
        .allowlist_var("fd_spad_.*")
    ...
    if wrapper_c_path.exists() {
        ...
        wrapper_build.compile("fdspadwrapper");
    }
}
