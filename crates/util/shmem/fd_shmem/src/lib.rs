...
//! It wraps the unsafe FFI bindings provided by `fd_shmem_sys`.
//!
...
pub enum PageSize {
    /// Normal 4KB pages
    Normal = fd_shmem_sys::FD_SHMEM_NORMAL_PAGE_SZ as isize,
    /// Huge 2MB pages
    Huge = fd_shmem_sys::FD_SHMEM_HUGE_PAGE_SZ as isize,
    /// Gigantic 1GB pages
    Gigantic = fd_shmem_sys::FD_SHMEM_GIGANTIC_PAGE_SZ as isize,
}
...
impl PageSize {
    ...
    pub fn log2_size(self) -> i32 {
        match self {
            PageSize::Normal => fd_shmem_sys::FD_SHMEM_NORMAL_LG_PAGE_SZ as i32,
            PageSize::Huge => fd_shmem_sys::FD_SHMEM_HUGE_LG_PAGE_SZ as i32,
            PageSize::Gigantic => fd_shmem_sys::FD_SHMEM_GIGANTIC_LG_PAGE_SZ as i32,
        }
    }
    ...
    pub fn from_raw(page_sz: u64) -> Option<Self> {
        match page_sz {
            x if x == fd_shmem_sys::FD_SHMEM_NORMAL_PAGE_SZ as u64 => Some(PageSize::Normal),
            x if x == fd_shmem_sys::FD_SHMEM_HUGE_PAGE_SZ as u64 => Some(PageSize::Huge),
            x if x == fd_shmem_sys::FD_SHMEM_GIGANTIC_PAGE_SZ as u64 => Some(PageSize::Gigantic),
            _ => None,
        }
    }
    ...
    pub fn as_str(self) -> &'static str {
        unsafe {
            let c_str = fd_shmem_sys::fd_shmem_page_sz_to_cstr(self as u64);
    ...
    pub fn from_str(s: &str) -> Option<Self> {
        ...
        let page_sz = unsafe { fd_shmem_sys::fd_cstr_to_shmem_page_sz(c_str.as_ptr()) };
...
pub enum JoinMode {
    /// Read-only access
    ReadOnly = fd_shmem_sys::FD_SHMEM_JOIN_MODE_READ_ONLY as isize,
    /// Read-write access
    ReadWrite = fd_shmem_sys::FD_SHMEM_JOIN_MODE_READ_WRITE as isize,
}
...
impl Drop for ShmemJoin {
    fn drop(&mut self) {
        let result =
            unsafe { fd_shmem_sys::fd_shmem_leave(self.join_handle, None, ptr::null_mut()) };
        if result != 0 {
            eprintln!(
...
impl FdShmem {
    ...
    pub fn numa_count() -> u64 {
        unsafe { fd_shmem_sys::fd_shmem_numa_cnt() }
    }

    /// Get the number of logical CPUs in the system
    pub fn cpu_count() -> u64 {
        unsafe { fd_shmem_sys::fd_shmem_cpu_cnt() }
    }

    /// Get the NUMA node index for a given CPU
    pub fn numa_idx_for_cpu(cpu_idx: u64) -> Option<u64> {
        let numa_idx = unsafe { fd_shmem_sys::fd_shmem_numa_idx(cpu_idx) };
        if numa_idx == u64::MAX {
            None
        } else {
            Some(numa_idx)
    ...
    pub fn cpu_idx_for_numa(numa_idx: u64) -> Option<u64> {
        let cpu_idx = unsafe { fd_shmem_sys::fd_shmem_cpu_idx(numa_idx) };
        if cpu_idx == u64::MAX {
            None
        } else {
            Some(cpu_idx)
    ...
    pub fn validate_numa_placement(
        ...
        let result = unsafe {
            fd_shmem_sys::fd_shmem_numa_validate(
    ...
    pub fn join(name: &str, mode: JoinMode, lock_pages: bool) -> ShmemResult<ShmemJoin> {
        ...
        let mut join_info = unsafe { core::mem::zeroed::<fd_shmem_sys::fd_shmem_join_info>() };

        let join_handle = unsafe {
            fd_shmem_sys::fd_shmem_join(
                c_name.as_ptr(),
                mode as i32,
        ...
        let join_mode = match join_info.mode {
            x if x == fd_shmem_sys::FD_SHMEM_JOIN_MODE_READ_ONLY as i32 => JoinMode::ReadOnly,
            x if x == fd_shmem_sys::FD_SHMEM_JOIN_MODE_READ_WRITE as i32 => JoinMode::ReadWrite,
    ...
    pub fn query_by_name(name: &str) -> ShmemResult<JoinInfo> {
        ...
        let result =
            unsafe { fd_shmem_sys::fd_shmem_join_query_by_name(c_name.as_ptr(), &mut join_info) };
    ...
    pub fn create_multi(
        ...
        let result = unsafe {
            fd_shmem_sys::fd_shmem_create_multi(
                c_name.as_ptr(),
                page_size as u64,
    ...
    pub fn unlink(name: &str, page_size: PageSize) -> ShmemResult<()> {
        ...
        let result = unsafe { fd_shmem_sys::fd_shmem_unlink(c_name.as_ptr(), page_size as u64) };
    ...
    pub fn info(name: &str, page_size: Option<PageSize>) -> ShmemResult<ShmemInfo> {
        ...
        let mut info = unsafe { core::mem::zeroed::<fd_shmem_sys::fd_shmem_info>() };

        let result = unsafe { fd_shmem_sys::fd_shmem_info(c_name.as_ptr(), page_sz, &mut info) };
    ...
    pub fn acquire(page_size: PageSize, page_count: u64, cpu_idx: u64) -> ShmemResult<*mut u8> {
        ...
        let ptr = unsafe {
            fd_shmem_sys::fd_shmem_acquire_multi(page_size as u64, 1, &page_count, &cpu_idx)
        };
    ...
    pub fn validate_name(name: &str) -> bool {
        ...
        let len = unsafe { fd_shmem_sys::fd_shmem_name_len(c_name.as_ptr()) };
        len > 0
    }
}
...
pub struct ShmemIterator {
    current: *const fd_shmem_sys::fd_shmem_join_info,
}

impl ShmemIterator {
    /// Create a new iterator over joined regions
    pub fn new() -> Self {
        let current = unsafe { fd_shmem_sys::fd_shmem_iter_begin() };
        Self { current }
    }
}
...
impl Iterator for ShmemIterator {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        // Advance to next
        self.current = unsafe { fd_shmem_sys::fd_shmem_iter_next(self.current) };
