...
//!
//! This wraps the FFI bindings provided by `fd_tpool_sys` and provides
...
pub mod pool {
    ...
    impl From<Options> for u64 {
        fn from(opts: Options) -> Self {
            let mut flags = 0u64;
            if opts.sleep {
                flags |= fd_tpool_sys::FD_TPOOL_OPT_SLEEP as u64;
            }
            flags
        }
    }
    ...
    pub struct ThreadPool {
        tpool: *mut fd_tpool_sys::fd_tpool_t,
        mem: *mut u8,
        layout: Layout,
        worker_max: usize,
        _marker: PhantomData<*mut fd_tpool_sys::fd_tpool_t>,
    }
    ...
    impl ThreadPool {
        ...
        pub fn new(worker_max: usize, options: Options) -> Result<Self, &'static str> {
            ...
            unsafe {
                let align = fd_tpool_sys::fd_tpool_align() as usize;
                let footprint = fd_tpool_sys::fd_tpool_footprint(worker_max as u64) as usize;
                ...
                let tpool =
                    fd_tpool_sys::fd_tpool_init(mem as *mut _, worker_max as u64, options.into());
        ...
        pub fn options(&self) -> Options {
            unsafe {
                ...
                Options {
                    sleep: (opt & fd_tpool_sys::FD_TPOOL_OPT_SLEEP as u64) != 0,
                }
            }
        }
        ...
        pub unsafe fn as_raw(&self) -> *mut fd_tpool_sys::fd_tpool_t {
            self.tpool
        }
    }
    ...
    impl Drop for ThreadPool {
        fn drop(&mut self) {
            unsafe {
                let returned_mem = fd_tpool_sys::fd_tpool_fini(self.tpool);
