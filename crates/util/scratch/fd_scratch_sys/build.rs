...
fn main() {
    ...
    let scratch_path = util_path.join("scratch");
    ...
    println!(
        ...
        scratch_path.join("fd_scratch.c").display()
    );
    println!(
        "cargo:rerun-if-changed={}",
        scratch_path.join("fd_scratch.h").display()
    );
    println!(
        "cargo:rerun-if-changed={}",
    ...
    build
        .file(scratch_path.join("fd_scratch.c"))
    ...
    let wrapper_path = out_path.join("scratch_wrapper.h");
...
"#,
    ...
    let mut bindgen = bindgen::Builder::default()
        ...
        .allowlist_function("fd_scratch_.*")
        .allowlist_function("fd_alloca_.*")
        .allowlist_type("fd_scratch_.*")
        .allowlist_type("fd_valloc_.*")
        .allowlist_var("FD_SCRATCH_.*")
        .allowlist_var("fd_scratch_.*")
    ...
    if wrapper_c_path.exists() {
        ...
        wrapper_build.compile("fdscratchwrapper");
    }
}
