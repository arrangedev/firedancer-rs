...
pub mod ethernet {
    ...
    pub const PAYLOAD_MAX: usize = libfd_net_sys::FD_ETH_PAYLOAD_MAX as usize;

    /// Min raw ethernet payload size: 46 bytes
    pub const PAYLOAD_MIN_RAW: usize = libfd_net_sys::FD_ETH_PAYLOAD_MIN_RAW as usize;
    ...
    impl MacAddress {
        ...
        pub fn from_str(s: &str) -> Result<Self, ParseError> {
            ...
            unsafe {
                let result =
                    libfd_net_sys::fd_cstr_to_mac_addr(c_str.as_ptr(), mac_addr.as_mut_ptr());
                if result.is_null() {
    ...
    pub fn calculate_fcs(data: &[u8]) -> u32 {
        unsafe {
            libfd_net_sys::fd_eth_fcs_append(
                libfd_net_sys::FD_ETH_FCS_APPEND_SEED,
...
pub mod ipv4 {
    ...
    pub fn parse_ipv4_addr(s: &str) -> Result<Ipv4Addr, ParseError> {
        ...
        unsafe {
            let result = libfd_net_sys::fd_cstr_to_ip4_addr(c_str.as_ptr(), &mut ip_addr);
            if result != 1 {
                return Err(ParseError::InvalidFormat);
            }
        }
...
pub mod pcap {
    /// From `fd_pcap.h`
    pub mod link_layer {
        pub const ETHERNET: u32 = libfd_net_sys::FD_PCAP_LINK_LAYER_ETHERNET;
        pub const USER0: u32 = libfd_net_sys::FD_PCAP_LINK_LAYER_USER0;
    }

    /// From `fd_pcap.h`
    pub mod iter_type {
        pub const ETHERNET: u32 = libfd_net_sys::FD_PCAP_ITER_TYPE_ETHERNET;
        pub const COOKED: u32 = libfd_net_sys::FD_PCAP_ITER_TYPE_COOKED;
    }

    /// From `fd_pcapng.h`
    pub mod pcapng {
        pub const LINKTYPE_ETHERNET: u32 = libfd_net_sys::FD_PCAPNG_LINKTYPE_ETHERNET;
        pub const FRAME_SIMPLE: u32 = libfd_net_sys::FD_PCAPNG_FRAME_SIMPLE;
        pub const FRAME_ENHANCED: u32 = libfd_net_sys::FD_PCAPNG_FRAME_ENHANCED;
        pub const FRAME_TLSKEYS: u32 = libfd_net_sys::FD_PCAPNG_FRAME_TLSKEYS;
    }
}
...
pub mod bits {
    ...
    pub fn floor_sqrt_u64(value: u64) -> u64 {
        unsafe { libfd_net_sys::fd_ulong_floor_sqrt(value) }
    }

    pub fn ceil_sqrt_u64(value: u64) -> u64 {
        unsafe { libfd_net_sys::fd_ulong_ceil_sqrt(value) }
    }

    pub fn round_sqrt_u64(value: u64) -> u64 {
        unsafe { libfd_net_sys::fd_ulong_round_sqrt(value) }
    }
}
