...
//!
//! // Create scratch allocator with 8KB memory and 16 frame depth
//! let mut allocator = ScratchAllocator::new(8192, 16).unwrap();
//!
//! // Use scoped frame for automatic cleanup
//! {
//!     let _frame = allocator.push_frame().unwrap();
//!     let memory = allocator.allocate(1024, 64).unwrap();
//!     // memory is automatically freed when _frame is dropped
//! }
//! ```

use core::ptr::{self, NonNull};
use libfd_scratch_sys::{
    self as sys, fd_scratch_alloc, fd_scratch_alloc_is_safe, fd_scratch_attach,
    fd_scratch_attach_is_safe, fd_scratch_detach, fd_scratch_detach_is_safe, fd_scratch_fmem_align,
    fd_scratch_fmem_footprint, fd_scratch_frame_free, fd_scratch_frame_used, fd_scratch_pop,
    fd_scratch_pop_is_safe, fd_scratch_prepare, fd_scratch_prepare_is_safe,
    fd_scratch_private_frame, fd_scratch_private_frame_cnt, fd_scratch_private_free,
    fd_scratch_publish, fd_scratch_publish_is_safe, fd_scratch_push, fd_scratch_push_is_safe,
    fd_scratch_smem_align, fd_scratch_trim, fd_scratch_trim_is_safe, fd_scratch_virtual, ulong,
};
...
/// RAII guard for a scratch frame; pops the current frame when dropped
pub struct ScratchFrame<'a> {
    allocator: &'a mut ScratchAllocator,
}

impl<'a> ScratchFrame<'a> {
    fn new(allocator: &'a mut ScratchAllocator) -> Self {
        Self { allocator }
    }

    pub fn allocator(&mut self) -> &mut ScratchAllocator {
        self.allocator
    }
}

impl<'a> Drop for ScratchFrame<'a> {
    fn drop(&mut self) {
        unsafe {
            if sys::fd_ != 0 {
                fd_scratch_pop();
...
impl ScratchAllocation {
    ...
    /// Trim the allocation to a smaller size.
    ///
    /// `end_ptr` should point to the first byte after the desired allocation
    pub fn trim(&mut self, new_size: usize) -> Result<(), ScratchError> {
        ...
        unsafe {
            if fd_scratch_trim_is_safe(end_ptr as *mut core::ffi::c_void) == 0 {
                return Err(ScratchError::InvalidState);
            }
            fd_scratch_trim(end_ptr as *mut core::ffi::c_void);
        }
...
impl ScratchAllocator {
    pub fn new(memory_size: usize, max_frames: usize) -> Result<Self, ScratchError> {
        ...
        let smem_align = unsafe { fd_scratch_smem_align() } as usize;
        let fmem_align = unsafe { fd_scratch_fmem_align() } as usize;

        let smem_footprint = unsafe { fd_scratch_smem_footprint(memory_size as ulong) } as usize;
        let fmem_footprint = unsafe { fd_scratch_fmem_footprint(max_frames as ulong) } as usize;
    ...
    fn attach(&mut self, memory_size: usize, max_frames: usize) -> Result<(), ScratchError> {
        unsafe {
            if fd_scratch_attach_is_safe() == 0 {
                return Err(ScratchError::InvalidState);
            }
    ...
    ///
    /// Returns a RAII guard that will pop the frame when dropped
    pub fn push_frame(&mut self) -> Result<ScratchFrame<'_>, ScratchError> {
        unsafe {
            if fd_scratch_push_is_safe() == 0 {
                return Err(ScratchError::TooManyFrames);
            }
            fd_scratch_push();
        }

        Ok(ScratchFrame::new(self))
    }

    /// Manually pop a frame
    ///
    /// not recommended - use push_frame() for RAII
    pub fn pop_frame(&mut self) -> Result<(), ScratchError> {
        unsafe {
            if fd_scratch_pop_is_safe() == 0 {
    ...
    pub fn allocate(
        ...
        unsafe {
            if fd_scratch_alloc_is_safe(alignment as ulong, size as ulong) == 0 {
                return Err(ScratchError::InsufficientSpace);
            }

            let ptr = fd_scratch_alloc(alignment as ulong, size as ulong);
            if ptr.is_null() {
    ...
    pub fn prepare_alloc(&mut self, alignment: usize) -> Result<DynamicAllocation, ScratchError> {
        ...
        unsafe {
            if fd_scratch_prepare_is_safe(alignment as ulong) == 0 {
                return Err(ScratchError::InsufficientSpace);
            }

            let ptr = fd_scratch_prepare(alignment as ulong);
            if ptr.is_null() {
                return Err(ScratchError::AllocationFailed);
            }
    ...
    pub fn frame_bytes(&self) -> usize {
        unsafe { fd_scratch_frame_used() as usize }
    }
...
impl Drop for ScratchAllocator {
    fn drop(&mut self) {
        unsafe {
            if fd_scratch_detach_is_safe() != 0 {
...
impl DynamicAllocation {
    ...
    pub fn publish(self, final_size: usize) -> Result<ScratchAllocation, ScratchError> {
        ...
        unsafe {
            if fd_scratch_publish_is_safe(end_ptr as *mut core::ffi::c_void) == 0 {
                return Err(ScratchError::PrepareMismatch);
            }
            fd_scratch_publish(end_ptr as *mut core::ffi::c_void);
        }
...
macro_rules! scratch_scope {
    ($allocator:expr, $block:block) => {{
        ...
        let mut allocator = _frame.allocator();
        $block
    }};
}
