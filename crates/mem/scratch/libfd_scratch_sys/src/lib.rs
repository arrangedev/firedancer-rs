...
pub unsafe fn fd_scratch_virtual() -> fd_valloc_t {
    fd_valloc_t {
        self_: ptr::null_mut(),
        vt: &fd_scratch_vtable,
    }
}
...
mod tests {
    ...
    fn test_scratch_basic_lifecycle() {
        ...
        unsafe {
            ...
            // Test safety checks after attach
            ...
            assert!(!ptr1.is_null());
            assert_eq!(ptr1 as usize % 64, 0); // Check alignment
            ...
            // Test pop (frees all allocations in current frame)
            assert_eq!(fd_scratch_pop_is_safe(), 1);
            fd_scratch_pop();
            assert_eq!(fd_scratch_frame_used(), 0);
            assert_eq!(fd_scratch_used(), 0); // All allocations freed
            ...
            // Test safety checks after detach
            assert_eq!(fd_scratch_attach_is_safe(), 1);
            assert_eq!(fd_scratch_detach_is_safe(), 0);
    ...
    fn test_scratch_reset() {
        ...
        unsafe {
            ...
            // Create multiple frames and allocations
            fd_scratch_push();
