...
fn main() {
    ...
    println!(
        "cargo:rerun-if-changed={}",
        bits_path.join("fd_bits_find_lsb.h").display()
    );
    println!(
        "cargo:rerun-if-changed={}",
        bits_path.join("fd_bits_find_msb.h").display()
    );
    println!(
        "cargo:rerun-if-changed={}",
        bits_path.join("fd_float.h").display()
    );
    println!(
        "cargo:rerun-if-changed={}",
        bits_path.join("fd_sat.h").display()
    );
    println!(
        "cargo:rerun-if-changed={}",
        bits_path.join("fd_uwide.h").display()
    );
    ...
    build.compile("fdbits");

    let mut bindgen = bindgen::Builder::default()
        .header(bits_path.join("fd_bits.h").to_string_lossy())
